...
public class MavenPluginConfigurationTranslator extends AbstractMavenPluginConfigurationTranslator {
	...
	private static final String CHECKSTYLE_DEFAULT_CONFIG_LOCATION = "config/" + CHECKSTYLE_DEFAULT_CONFIG_FILE_NAME;
	/** checkstyle maven plugin artifactId */
	private static final Map<String, String> PATTERNS_CACHE = new HashMap<>();
	private static final String CHECKSTYLE_DEFAULT_CONFIG_FILE_HEADER = "<?xml version=\"1.0\"?>\n"
	...
	private MavenPluginConfigurationTranslator(final IMaven maven, final MavenProject mavenProject,
			final MojoExecution mojoExecution, final IProject project, final URI basedirUri,
			final Path workingDirectory, final IProgressMonitor monitor) throws CoreException {
	...
	public String getSuppressionsFile() throws CheckstylePluginException, CoreException {
		...
		final Path suppressionsFile = workingDirectory
				.resolve("checkstyle-suppressions-" + sanitizeFilename(getExecutionId()) + ".xml");
		try (InputStream inputStream = suppressionsLocation.openStream()) {
			copyIfChanged(inputStream, suppressionsFile);
		} catch (final IOException e) {
			LOG.error("Could not copy suppressions file {}", suppressionsLocation, e);
			throw new CheckstylePluginException(
	...
	public String getSuppressionsFileExpression() throws CoreException {
		return getParameterValue("suppressionsFileExpression", String.class, "checkstyle.suppressions.file");
	}

	public void updateCheckConfigWithIncludeExcludePatterns(final ProjectConfigurationWorkingCopy pcWorkingCopy,
			final ICheckConfiguration checkCfg) throws CheckstylePluginException, CoreException {
		final FileSet fs = new FileSet("java-sources-" + sanitizeFilename(getExecutionId()), checkCfg);
		fs.setEnabled(true);
	...
	/**
	 * Get the {@literal includeTestSourceDirectory} element value if present in the
	 * configuration.
	 * 
	...
	private boolean isIncludeTestSourceDirectory() throws CoreException {
		return getParameterValue("includeTestSourceDirectory", Boolean.class, Boolean.FALSE);
	}

	private boolean isIncludeResources() throws CoreException {
		return getParameterValue("includeResources", Boolean.class, Boolean.TRUE);
	}

	private boolean isIncludeTestResources() throws CoreException {
		return getParameterValue("includeTestResources", Boolean.class, Boolean.TRUE);
	}
	...
	private String getConfigLocation() throws CoreException {
		return getParameterValue("configLocation", String.class, CHECKSTYLE_DEFAULT_CONFIG_LOCATION);
	}

	private URL getHeaderLocation() throws CoreException {
		String headerLocation = getParameterValue("headerLocation", String.class, "LICENSE.txt");
		if ("config/maven_checks.xml".equals(getConfigLocation()) && "LICENSE.txt".equals(headerLocation)) {
	...
	private URL getSuppressionsLocation() throws CoreException {
		String suppressionsLocation = getParameterValue("suppressionsLocation", String.class);
		if (suppressionsLocation == null) {
			suppressionsLocation = getParameterValue("suppressionsFile", String.class);
		}
		if (suppressionsLocation == null) {
			return null;
		}
	...
	private List<String> getSourceDirectories() throws CoreException {
		final List<String> sourceDirectories = new ArrayList<>(getParameterList("sourceDirectories", String.class));
		final String sourceDirectory = getParameterValue("sourceDirectory", String.class);
	...
	private List<String> getTestSourceDirectories() throws CoreException {
		...
		final String testSourceDirectory = getParameterValue("testSourceDirectory", String.class);
	...
	private List<FileMatchPattern> getIncludesExcludesFileMatchPatterns()
		...
		/**
		 * Step 2). Get all the includes patterns add them for all source roots. NOTES:
		 * - Since the eclipse-cs excludes override (or more correctly the later ones)
		 * the includes, we make sure that if no include patterns are specified we at
		 * least put the source folders. - Also, we use relative path to project root.
		 */
		...
		for (final String folder : sourceFolders) {
			...
			if (!includePatterns.isEmpty()) {
				patterns.addAll(
						this.normalizePatternsToCheckstyleFileMatchPattern(includePatterns, folderRelativePath, true));
			} else {
		...
		/**
		 * Step 3). Get all the excludes patterns add them for all source roots. NOTES:
		 * - Since the eclipse-cs excludes override (or more correctly the later ones)
		...
		if (this.isIncludeResources()) {
			final List<String> resourceIncludePatterns = this.getResourceIncludes();
			for (final Resource resource : getMavenProject().getBuild().getResources()) {
				final String folderRelativePath = relativize(resource.getDirectory());
				patterns.addAll(this.normalizePatternsToCheckstyleFileMatchPattern(resourceIncludePatterns,
						folderRelativePath, true));
			}

			final List<String> resourceExcludePatterns = this.getResourceExcludes();
			for (final Resource resource : getMavenProject().getBuild().getResources()) {
				final String folderRelativePath = relativize(resource.getDirectory());
				patterns.addAll(this.normalizePatternsToCheckstyleFileMatchPattern(resourceExcludePatterns,
						folderRelativePath, false));
			}
		}

		if (this.isIncludeTestResources()) {
			final List<String> resourceIncludePatterns = this.getResourceIncludes();
			for (final Resource resource : getMavenProject().getBuild().getTestResources()) {
				if (!resource.getExcludes().isEmpty() || !resource.getIncludes().isEmpty()) {
					// ignore resources that have ex/includes for now
					continue;
				}
				final String folderRelativePath = "^"
						+ this.basedirUri.relativize(new File(resource.getDirectory()).toURI()).getPath();
				patterns.addAll(this.normalizePatternsToCheckstyleFileMatchPattern(resourceIncludePatterns,
						folderRelativePath, true));
			}
	...
	private String relativize(final String folder) {
		...
		if (file.isAbsolute()) {
			folderRelativePath += this.basedirUri.relativize(file.toURI()).getPath();
		} else {
	...
	private List<FileMatchPattern> normalizePatternsToCheckstyleFileMatchPattern(final List<String> patterns,
			final String relativePath, final boolean setIsIncludePatternFlag) throws CheckstylePluginException {
	...
	/**
	 * Get the include or exclude patterns from the plugin configuration. NOTE: this
	 * has to be unfortunately transformed, as Maven Plugin configuration supports
	 * ANT style pattern but the Eclipse-CS requires java regex.
	 * 
	 * @param elemName the parent element name (e.g.
	 *                 {@code <includes> or <excludes>} .
	 * @return a {@code List} of include patterns.
	 * @throws CoreException
	 */
	private List<String> getPatterns(final String elemName) throws CoreException {
	...
	public Properties getConfiguredProperties() throws CoreException, CheckstylePluginException {
		...
		if (propertiesLocation != null) {
			...
			if (url == null) {
				throw new CheckstylePluginException(
						String.format("Failed to resolve propertiesLocation [%s]", propertiesLocation));
			}
			try {
				properties.load(url.openStream());
			} catch (final IOException e) {
				throw new CheckstylePluginException(
						String.format("Failed to LOAD properties from propertiesLocation [%s]", propertiesLocation));
			}
		}
		return properties;
	}
	...
	public static List<MavenPluginConfigurationTranslator> newInstance(final IMaven maven,
			final AbstractMavenPluginProjectConfigurator<?> configurator, final MavenProject mavenProject,
			final MavenPluginWrapper mavenPlugin, final IProject project, final IProgressMonitor monitor)
			throws CoreException {
		final List<MavenPluginConfigurationTranslator> m2csConverters = new ArrayList<>();
		for (final MojoExecution execution : mavenPlugin.getMojoExecutions()) {
			final Path path = project.getWorkingLocation(configurator.getId()).toFile().toPath();
			m2csConverters.add(new MavenPluginConfigurationTranslator(maven, mavenProject, execution, project,
