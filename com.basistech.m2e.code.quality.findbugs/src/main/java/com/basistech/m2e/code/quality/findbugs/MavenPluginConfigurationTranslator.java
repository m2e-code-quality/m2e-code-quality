...
public class MavenPluginConfigurationTranslator {
	...
	private MavenPluginConfigurationTranslator(
		...
		this.resourceResolver = ResourceResolver
				.newInstance(configurator.getPluginClassRealm(session,
	...
	public void setIncludeFilterFiles(final UserPreferences prefs)
			throws CoreException {
		final String includeFilterFile = this.configurator.getParameterValue(
				INCLUDE_FILTER_FILE, String.class, session, execution);
		...
		final Map<String, Boolean> curIncludeFilteredFiles = prefs
	...
	public void setExcludeFilterFiles(final UserPreferences prefs)
		...
		final String excludeFilterFile = this.configurator.getParameterValue(
				EXCLUDE_FILTER_FILE, String.class, session, execution);
		// don't do anything if null
		...
		final Map<String, Boolean> curExcludeFilteredFiles = prefs
	...
	public void setBugCatagories(final UserPreferences prefs)
		...
		final String bugCatagories = this.configurator.getParameterValue(
				BUG_CATEGORIES, String.class, session, execution);
		if (bugCatagories == null) {
			log.debug("bugCatagories is null");
			return;
		}
		List<String> addBugCatagoriesList = Arrays.asList(StringUtils.split(
				bugCatagories, ","));
		List<String> availableBugCategories = new LinkedList<String>(
	...
	public void setEffort(final UserPreferences prefs) throws CoreException {
		String effort = this.configurator.getParameterValue(EFFORT,
	...
	public void setPriority(final UserPreferences prefs) throws CoreException {
		final String priority = this.configurator.getParameterValue(PRIORITY,
				String.class, session, execution);
		if (priority == null) {
	...
	public void setOmitVisitors(final UserPreferences prefs)
		...
		final String omitVisitors = this.configurator.getParameterValue(
				OMIT_VISITORS, String.class, session, execution);
		if (omitVisitors == null) {
			log.debug("omitVisitors is null");
			return;
		}
		List<String> detectorsList = Arrays.asList(StringUtils.split(
				omitVisitors, ","));
		final DetectorFactoryCollection dfc = DetectorFactoryCollection
				.instance();
	...
	public void setThreshold(final UserPreferences prefs) throws CoreException {
		final String threshold = this.configurator.getParameterValue(
				THRESHOLD, String.class, session, execution);
		if (threshold == null) {
	...
	public void setVisitors(final UserPreferences prefs) throws CoreException {
		final String visitors = this.configurator.getParameterValue(VISITORS,
				String.class, session, execution);
		if (visitors == null) {
			return;
		}
	...
	private void copyUrlResourceToProject(final String resc,
		...
		final File newLocationFile = new File(this.project.getLocationURI()
				.getPath(), newLocation);
		try {
