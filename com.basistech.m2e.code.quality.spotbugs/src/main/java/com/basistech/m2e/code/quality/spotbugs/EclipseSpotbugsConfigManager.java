 ...
 * and config file location for SpotBugs.
 *
 */
public class EclipseSpotbugsConfigManager extends EclipseConfigManager {

	private static final Logger LOG =
	        LoggerFactory.getLogger(EclipseSpotbugsConfigManager.class);

	private EclipseSpotbugsConfigManager(final FindBugsNature fbNature) {
		super(fbNature, ECLIPSE_FB_NATURE_ID, ECLIPSE_FB_PREFS_FILE);
	}

	public static EclipseSpotbugsConfigManager newInstance(
	        final IProject project) {
		LOG.debug("entering newInstance");
		Preconditions.checkNotNull(project);
		final FindBugsNature fbNature = new FindBugsNature();
		fbNature.setProject(project);
		return new EclipseSpotbugsConfigManager(fbNature);
	}
}
